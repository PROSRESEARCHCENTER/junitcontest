...
public class GaussianCore2DNorm extends FunctionCore {
	...
	super(2, 4, new double[] {0., 1., 0., 1.});
	setTitle("GaussianCore2DNorm::"+str);
	providesNormalization = true;

	String[] names = new String[] { "meanX", "sigmaX", "meanY", "sigmaY" };
	setParameterNames(names);
    }

    public GaussianCore2DNorm(String str, double[] pVal) {
	super(2, 4, pVal);
	setTitle("GaussianCore2DNorm::"+str);
	providesNormalization = true;
	...
	return Math.exp( -Math.pow( var[0] - p[0], 2 )/(2*Math.pow( p[1], 2 )) ) *
	       Math.exp( -Math.pow( var[1] - p[2], 2 )/(2*Math.pow( p[3], 2 )) );
    }
	...
	return new double[] { functionValue(var)*(-2.)*(var[0] - p[0])/(2*Math.pow( p[1], 2 )), 
			      functionValue(var)*(-2.)*(var[1] - p[2])/(2*Math.pow( p[3], 2 )) };
    }
	...
	return new double[] {  f*2.*(var[0] - p[0])/(2*Math.pow( p[1], 2 )),
			       f*2.*(var[0] - p[0])/(2*Math.pow( Math.abs(p[1]), 3 )),
			       f*2.*(var[1] - p[2])/(2*Math.pow( p[3], 2 )),
			       f*2.*(var[1] - p[2])/(2*Math.pow( Math.abs(p[3]), 3 )) };
    }
	...
	return Math.sqrt( Math.PI/2 )*p[1]*(erf( (xMax[0]-p[0])/(r2*p[1]) ) - erf( (xMin[0]-p[0])/(r2*p[1]) ) ) *
	       Math.sqrt( Math.PI/2 )*p[3]*(erf( (xMax[1]-p[2])/(r2*p[3]) ) - erf( (xMin[1]-p[2])/(r2*p[3]) ) );
    }
