...
public class HepRepAdapter implements HepRep {
    ...
    /**
     * Create CORBA wrapper for HepRep
...
//        Object obj = instanceTrees.get(factory.createHepRepTreeID(instanceTree.getName(), instanceTree.getVersion()));
//        System.out.println(obj.hashCode()+" "+obj);
//        System.out.println("Adding INSTANCE tree '"+instanceTree.getName());
    ...
    public HepRepInstanceTree getInstanceTreeTop(String name, String version) {
        ...
        if (instanceTree == null) {
            instanceTree = new HepRepInstanceTreeAdapter(this,
                        hepRep.getInstanceTreeTop(name, version));
            // FIXME check this with Joe
//            System.out.println("Request INSTANCE treeTOP '"+name+"' '"+version+"'");
//            System.out.println("Adding  INSTANCE treeTOP '"+instanceTree.getName()+"' '"+instanceTree.getVersion()+"' "+instanceTree.hashCode());
            instanceTrees.put(factory.createHepRepTreeID(instanceTree.getName(), instanceTree.getVersion()), instanceTree);
        }
        return instanceTree;
    }
    ...
    public HepRepInstanceTree getInstances(String name, String version,
                                           String[] typeNames) {
        // FIXME, not cached
        return new HepRepInstanceTreeAdapter(this,
                        hepRep.getInstances(name, version, typeNames));
    }
    ...
    public HepRepInstanceTree getInstancesAfterAction(
                                    ...
                                    String[] invertAtts) {
        hep.graphics.heprep.corba.idl.HepRepAction[] hepRepActions =
            new hep.graphics.heprep.corba.idl.HepRepAction[actions.length];
        for (int i=0; i<hepRepActions.length; i++) {
            hepRepActions[i] =
                new hep.graphics.heprep.corba.idl.HepRepAction(actions[i].getName(), actions[i].getExpression());
        }
        // FIXME, not cached
        return new HepRepInstanceTreeAdapter(this,
