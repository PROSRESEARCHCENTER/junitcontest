...
//      JOptionPane.showOptionDialog(source,message,"Error...",JOptionPane.DEFAULT_OPTION,JOptionPane.ERROR_MESSAGE,null,options,options[0]);
      showErrorDialog(source, message, detail, null);
   }
    ...
    * @param detail The exception that caused the error (may be null)
    */
   public static int showErrorDialog(Component source, Object message, final Throwable detail, Object[] options)
   {
      final JButton details = new JButton("Details...");
      details.setEnabled(detail != null);
      details.addActionListener(new ActionListener()
      ...
      Object[] opts;
      if (options == null) {
        opts = new Object[] { "OK" , details };
      } else {
        opts = Arrays.copyOf(options, options.length+1);
        opts[options.length] = details;
      }
      return JOptionPane.showOptionDialog(source,message,"Error...",JOptionPane.DEFAULT_OPTION,JOptionPane.ERROR_MESSAGE,null,opts,opts[0]);
   }
